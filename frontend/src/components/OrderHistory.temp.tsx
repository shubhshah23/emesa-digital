...
const OrderHistory = ({ user, onBack }: OrderHistoryProps) => {
  ...
  const [latestCounterOffers, setLatestCounterOffers] = useState<{ [orderId: number]: { amount: number, sender: string, admin_notes?: string } | null }>({});
  ...
  useEffect(() => {
    const fetchCounterOffers = async () => {
      const offers: { [orderId: number]: { amount: number, sender: string, admin_notes?: string } | null } = {};
                          ...
                          {isExpanded && (
                            <CardContent>
                              ...
                              <div className="mt-6 p-4 bg-gray-100 rounded-md border border-gray-200">
                                <h5 className="font-medium mb-2 text-gray-800">Status Update</h5>
                                ...
                                {['negotiation', 'quotation_sent', 'awaiting_payment'].includes(order.status) && latestCounterOffers[order.id] && (
                                  <div className="mt-4 p-4 bg-yellow-50 border border-yellow-100 rounded-md">
                                    ...
                                    {latestCounterOffers[order.id]?.sender === 'admin' && (
                                      <div className="flex gap-2 mt-2">
                                        <Button variant="outline" size="sm" className="border-gray-300 text-gray-700 hover:bg-gray-200" onClick={() => handleAcceptCounterOffer(order.id)}>Accept Counter Offer</Button>
                                        <Button variant="secondary" size="sm" className="border-gray-300" onClick={() => setShowCounterOfferModal(order.id)}>Counter Offer</Button>
                                      </div>
                                    )}
                                  </div>
                                )}
                                ...
                                {order.status === 'accepted' && (
                                  <div className="mt-4 p-4 bg-gray-100 border border-gray-200 rounded-md text-black font-medium">Order accepted. Please complete payment to proceed.</div>
                                )}

                                {order.status === 'awaiting_payment' && !order.payment_confirmed && (
                                  <div>
                                    <div className="mt-4 p-4 bg-gray-100 border border-gray-200 rounded-md text-black font-medium">Order accepted. Please complete payment to proceed.</div>
                                    <div className="mb-2">Total Amount: <span className="font-semibold">₹{order.price_estimate || order.target_price}</span></div>
                                    <Button variant="default" size="lg" className="bg-black hover:bg-gray-900 text-white rounded-md" onClick={() => handlePayNow(order.id)}>Pay Now</Button>
                                  </div>
                                )}

                                {order.payment_confirmed && (
                                  <div className="mt-4 p-4 bg-gray-100 border border-gray-200 rounded-md text-black font-medium">Payment Confirmed</div>
                                )}
                              </div>
                                        ...
                                        <div>
                                          ...
                                          <div className="mb-8 space-y-2">
                                            ...
                                            <div className="flex justify-between text-base">
                                              ...
                                              <span>₹{order.price_estimate || order.target_price}</span>
                                            </div>
                                          </div>
